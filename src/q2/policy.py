...
class Policy:
    ...
    def __post_init__(self):
        ...
        reserve = np.zeros(len(self.grid)) if self.reserve is None else np.array(self.reserve, dtype=float, copy=True)
        if reserve.shape != (len(self.grid),) or not np.isfinite(reserve).all():
            raise ValueError("动态SOC安全裕度必须是与策略等长的有限一维数组")
        if (reserve < 0).any() or (reserve > E_MAX-E_MIN+PHYSICAL_TOL).any():
            raise ValueError("动态SOC安全裕度超出可用容量")
        reserve.setflags(write=False)
    ...
    def first(self, periods=144):
        return Policy(self.grid[:periods], self.charge_cap[:periods], self.discharge_cap[:periods],
                      self.reserve[:periods])


def respond(net, previous_soc, grid, charge_cap, discharge_cap, reserve=0.0):
    """只接收本时段观测；没有未来数组、场景库或优化器接口。单位全部kWh。"""
    if not np.isfinite([net, previous_soc, grid, charge_cap, discharge_cap, reserve]).all():
    ...
    if not 0 <= reserve <= E_MAX-E_MIN+PHYSICAL_TOL:
        raise ValueError("动态SOC安全裕度不合法")
    deficit = net - grid
    charge = min(charge_cap, max(-deficit, 0), max((E_MAX - previous_soc) / ETA_C, 0))
    discharge = min(discharge_cap, max(deficit, 0),
                    max(ETA_D * (previous_soc - E_MIN - reserve), 0))
...
def replay(policy, net, initial_soc):
    ...
    for t, current_net in enumerate(net):
        ...
        output[t] = respond(current_net, state, policy.grid[t], cp, rp, policy.reserve[t])
        state = output[t, 4]
    result = dict(zip(("charge", "discharge", "emergency", "unused", "soc"), output.T))
    validate_replay(policy.grid, net, initial_soc, result, reserve=policy.reserve)
    return result


def validate_replay(grid, net, initial_soc, result, *, rigid=False, reserve=None):
    ...
    reserve = np.zeros(len(e)) if reserve is None else np.asarray(reserve, dtype=float)
    if reserve.shape != e.shape or not np.isfinite(reserve).all() or (reserve < 0).any():
        raise ValueError("回放动态SOC安全裕度不合法")
    previous = np.r_[initial_soc, e[:-1]]
    available = ETA_D*np.maximum(previous-E_MIN-reserve, 0)
    if not rigid and np.any(r > available+PHYSICAL_TOL):
        raise ValueError("回放放电超过动态SOC安全裕度后的可用电量")
