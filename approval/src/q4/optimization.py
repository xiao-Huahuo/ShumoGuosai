...
def build_matrix(scenarios: Scenarios, initial: float, *, reference: np.ndarray | None = None,
    ...
    if not E_MIN - TOL <= initial <= E_MAX + TOL or scenarios.radius < 0 or not np.isfinite(scenarios.radius):
...
def dual_bound(row_dual, col_dual, constraints, bounds):
    """有限界才进入对偶内积；无限界处只允许solver容差内的舍入残差。"""
    value, invalid_multiplier = 0.0, 0.0
    for multipliers, lo, hi in ((row_dual, constraints.lb, constraints.ub),
                                (col_dual, bounds.lb, bounds.ub)):
        positive, negative = multipliers > 0, multipliers < 0
        invalid = (positive & ~np.isfinite(lo)) | (negative & ~np.isfinite(hi))
        invalid_multiplier = max(invalid_multiplier, float(np.max(abs(multipliers[invalid]), initial=0)))
        pos = positive & np.isfinite(lo)
        neg = negative & np.isfinite(hi)
        value += float(multipliers[pos] @ lo[pos] + multipliers[neg] @ hi[neg])
    return value, invalid_multiplier
...
def solve_dual(scenarios: Scenarios, initial: float, config: Config, *, reference=None, today=None) -> Solution:
    ...
    lower_bound, invalid_multiplier = dual_bound(row_dual, col_dual, constraints, bounds)
    info = solver.getInfo()
    dual_feasibility = max(invalid_multiplier, float(info.max_dual_infeasibility))
    certificate_error = abs(raw_value - lower_bound)
    valid = bool(feasibility <= TOL and duality_error <= 2e-4 and certificate_error <= 2e-4 and dual_feasibility <= 1e-8)
    ...
    audit = dict(status='Optimal', solver='HiGHS_finite_support_Wasserstein_LP', solver_version=solver.version(),
                 ...
                 lp_certificate_error=certificate_error, dual_feasibility_error=dual_feasibility,
                 native_primal_dual_objective_error=float(info.primal_dual_objective_error),
                 simultaneous_charge_removed=float(overlap.sum()),
