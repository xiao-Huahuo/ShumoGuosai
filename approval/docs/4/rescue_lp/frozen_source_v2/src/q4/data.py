...
from .config import ATTACHMENTS, DT, Q2_RUN, ROOT, production_sources, write_csv, write_json
...
class Inputs:
    ...
    def price_residual_path(self, origin_day: int, hour: int, length: int, *, cutoff: int) -> np.ndarray:
        """完整窗口已实现，且跨午夜始终使用同一次0:00预测。"""
        start = origin_day * 144 + hour * 6
        if origin_day < 7 or start + length > cutoff or start + length > self.actual_prices.size:
            raise ValueError("残差窗口尚未完整实现或历史起点非法")
        forecast = self.point_price(origin_day, hour, length)
        actual = self.actual_prices.reshape(-1)[start:start + length]
    ...
    def joint_history(self, day: int, hour: int, length: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for history_day, net_error in zip(days, net_errors):
            ...
            if start + length > cutoff:
                continue
            price_error = self.price_residual_path(int(history_day), hour, length, cutoff=cutoff)
...
def read_inputs(processed: Path | None = None, q2_run: Path = Q2_RUN) -> Inputs:
    ...
    provenance = {
        ...
        "q3_provenance": q3.provenance,
        "production_code_sha256": {str(p.relative_to(Path(__file__).resolve().parents[1])): hashlib.sha256(p.read_bytes()).hexdigest()
                                   for p in production_sources() if not p.name.startswith("test_")},
        ...
        "price_forecasts_array_sha256": hashlib.sha256(forecasts.values.tobytes()).hexdigest(),
        "q2_frozen_dispatch_sha256": hashlib.sha256((q2_run / "processed/main/dispatch.csv").read_bytes()).hexdigest(),
