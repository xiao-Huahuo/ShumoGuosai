...
from .fast_solver import (build_matrix,build_matrix_v2_free,build_matrix_v2_2bit,matrix_digest,
                          seed_vector,matrix_error,solve_fast)
...
def capture(run: Path, date: str, output: Path) -> None:
    ...
    today=108;prices=data.prices[(36+np.arange(144))%144];terminal=saved['audit']['terminal']
    with patch.object(Config,'signature',return_value=execution['signature']):
        signature=request_digest(config,scenes.load,scenes.pv,scenes.weights,prices,initial,
            reference,today,terminal,previous_net(data,day,6),None,None,None)
    if signature!=saved['signature']:raise ValueError('固定节点输入签名重建不符')
    with (output/'input.dat').open('wb') as stream:
        np.savez_compressed(stream,load=scenes.load,pv=scenes.pv,weights=scenes.weights,net=scenes.net,prices=prices,
            initial=initial,reference=reference,today=today,terminal=terminal,
            grid=saved['policy']['grid'],charge_cap=saved['policy']['charge_cap'],discharge_cap=saved['policy']['discharge_cap'])
    policy_sha=hashlib.sha256(json.dumps(saved['policy'],sort_keys=True,separators=(',',':')).encode()).hexdigest()
    matrices={}
    for name,builder in (('legacy',build_matrix),('v2_free',build_matrix_v2_free),('v2_2bit',build_matrix_v2_2bit)):
        matrix=builder(scenes,prices,initial,reference=reference,today=today,terminal=terminal)
        matrices[name]={'digest':matrix_digest(*matrix[:4]),'variables':len(matrix[0]),
                        'constraints':int(matrix[3].A.shape[0]),'binaries':int(matrix[2].sum())}
    input_sha=hashlib.sha256((output/'input.dat').read_bytes()).hexdigest()
    record={'run':str(run),'date':date,'hour':6,'signature':signature,'today':today,
        'source_audit':saved['audit'],'scenario_audit':scenes.audit,'policy_sha256':policy_sha,
        'input_sha256':input_sha,'matrices':matrices}
    write_json(output/'input.json',record)
    manifest_path=output.parent/'matrix_manifest.json'
    manifest=json.loads(manifest_path.read_text(encoding='utf-8')) if manifest_path.exists() else {'nodes':{}}
    manifest['nodes'][date]=record;write_json(manifest_path,manifest)


def policy_sha(policy: Policy) -> str:
    payload={key:getattr(policy,key).tolist() for key in ('grid','charge_cap','discharge_cap')}
    return hashlib.sha256(json.dumps(payload,sort_keys=True,separators=(',',':')).encode()).hexdigest()


def benchmark(folder: Path, formulation: str, seconds: float, threads: int, output: Path,
              solver_focus: str='default') -> dict:
    data=np.load(folder/'input.dat');scenes=Scenarios(data['load'],data['pv'],data['weights'],{})
    if 'net' in data.files:np.testing.assert_array_equal(data['net'],scenes.net)
    initial=float(data['initial']);terminal=float(data['terminal']);prices=data['prices'];ref=data['reference']
    today=int(data['today']) if 'today' in data.files else 108
    seed=Policy(data['grid'],np.full(len(prices),CAP),data['discharge_cap'])
    cfg=Config(gap=.03,seconds=seconds,solver_threads=threads,solver_backend='highs',
               formulation=formulation,solver_focus=solver_focus)
    records=[];started=time.perf_counter()
    seed_responses=[replay(seed,path,initial) for path in scenes.net]
    records.append({'seconds':0.,'objective':objective(seed,seed_responses,scenes.weights,prices,ref,today,terminal),
                    'lower_bound':None,'gap':None,'reliable':False,'policy_sha256':policy_sha(seed)})
    def observe(policy,responses,audit):
        records.append({'seconds':audit.get('seconds',time.perf_counter()-started),'objective':audit['objective'],
            'optimizer_objective':audit.get('optimizer_objective'),'lower_bound':audit.get('lower_bound'),
            'gap':audit.get('gap'),'reliable':audit.get('reliable'),'node_count':audit.get('node_count'),
            'simplex_iterations':audit.get('simplex_iterations'),'barrier_iterations':audit.get('barrier_iterations'),
            'policy_sha256':policy_sha(policy)})
    policy,responses,audit=solve_fast(scenes,prices,initial,cfg,reference=ref,today=today,terminal=terminal,
        seed=seed,seconds=seconds,progress=observe,formulation=formulation)
    audit.update({'wall_seconds':time.perf_counter()-started,'backend':'highs',
        'formulation_version':formulation,'policy_sha256':policy_sha(policy),
        'solver_focus':solver_focus,'trajectory':records,'fixture':folder.name,'original_replay_verified':True})
    output.parent.mkdir(parents=True,exist_ok=True);write_json(output,audit)
    return audit
...
def probe(folder: Path, method: str, seconds: float) -> dict:
    ...
    initial=float(data['initial']);terminal=float(data['terminal']);prices=data['prices'];ref=data['reference']
    today=int(data['today']) if 'today' in data.files else 108
    f,b,integer,c,idx=build_matrix(scenes,prices,initial,reference=ref,today=today,terminal=terminal)
...
if __name__=='__main__':
    ...
    parser.add_argument('--seconds',type=float,default=60.);parser.add_argument('--benchmark-output',type=Path)
    parser.add_argument('--formulation',choices=('legacy','v2_free'),default='v2_free');parser.add_argument('--threads',type=int,default=4)
    ...
    elif args.benchmark_output:print(json.dumps(benchmark(args.folder,args.formulation,args.seconds,args.threads,args.benchmark_output,args.solver_focus)))
    elif args.method:print(json.dumps(probe(args.folder,args.method,args.seconds)))
    else:parser.error('必须指定capture、benchmark-output或method')
