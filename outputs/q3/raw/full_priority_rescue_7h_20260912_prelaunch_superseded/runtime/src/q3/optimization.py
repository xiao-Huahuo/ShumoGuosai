...
def accept_feasible_incumbent(policy: Policy | None, scenarios: Scenarios, prices: np.ndarray,
                              initial: float, config: Config, *, reference: np.ndarray | None,
                              today: int, terminal: float, previous_net: float, audit: dict,
                              hard_seconds: float, source: str) -> Solution:
    """比赛救援仅接受经原 replay 与完整 legacy 矩阵复核的当前可行策略。"""
    if policy is None:
        raise LimitedSolve({**audit,'accepted':False,'acceptance_rejection':'no_feasible_incumbent'})
    net=scenarios.net;prices=np.asarray(prices,dtype=float);count=len(prices)
    upper=scenarios.load.max(0)/6+CAP
    domain_error=max(float(np.max(-policy.grid)),float(np.max(policy.grid-upper)),
                     float(np.max(-policy.charge_cap)),float(np.max(policy.charge_cap-CAP)),
                     float(np.max(-policy.discharge_cap)),float(np.max(policy.discharge_cap-CAP)),0.)
    if any(values.shape!=(count,) or not np.isfinite(values).all()
           for values in (policy.grid,policy.charge_cap,policy.discharge_cap)) or domain_error>TOL:
        raise LimitedSolve({**audit,'accepted':False,'acceptance_rejection':'policy_domain_invalid',
                            'policy_domain_error':domain_error})
    responses=[replay(policy,path,initial,mode=config.feedback,previous_net=previous_net) for path in net]
    value=objective(policy,responses,scenarios.weights,prices,reference,today,terminal)
    from .fast_solver import build_matrix,seed_vector,matrix_error,matrix_digest
    matrix=build_matrix(scenarios,prices,initial,reference=reference,today=today,
                        terminal=terminal,fixed_policy=policy if audit.get('fixed_policy') else None)
    coefficients,bounds,integer,constraints,indices=matrix
    vector=seed_vector(policy,scenarios,prices,initial,indices,len(coefficients),reference)
    ...
    accepted={**audit,'status':'hard_limit_feasible','objective':value,'upper_bound':value,'optimizer_objective':raw_value,
        ...
        'policy_domain_error':domain_error,'linear_feasibility_error':feasibility,
...
def reached_hard_limit(audit: dict, elapsed: float, budget: float) -> bool:
    return 'time limit' in str(audit.get('status','')).lower() or elapsed>=budget-max(.05,.02*budget)
...
def solve(scenarios: Scenarios, prices: np.ndarray, initial: float, config: Config, *,
          ...
          checkpoint: Path | None = None, max_slices: int | None = 1,
          hard_seconds: float | None = None) -> Solution:
    started = time.perf_counter()
    budget=hard_seconds if config.production_rescue else None
    if config.production_rescue and (budget is None or budget<=0):
        raise ValueError('production rescue必须显式传入节点总hard wall')
    def remaining() -> float:
        return max(.01,(budget-(time.perf_counter()-started)) if budget is not None else config.seconds)
    ...
    relaxed = make_model(min(remaining(),max(.01,(budget/4 if budget is not None else config.seconds/4))))
    ...
    if restored_lower>relaxed_lower:
        ...
        lower_provenance={'kind':'external_original_recourse_LP','solver':'SCIP','value':lower,
            'valid_for':'original_Q3_policy_problem'}
    candidates = []
    ...
    if fixed is None and config.feedback == 'aggregate' and value-lower > max(TOL, config.gap*abs(value)):
        ...
        candidate = refine(seed, net, prices, weights, initial, reference, today, terminal, upper,
                           min(1.,remaining(),(budget/10 if budget is not None else config.seconds/10)))
    ...
    if fixed is None and config.feedback=='aggregate' and config.gap>0:
        ...
        if checkpoint:
            save_snapshot(checkpoint/'latest.json',signature,seed,{
                ...
                'bound_sources':[lower_provenance],'formulation_version':config.formulation})
        slice_seconds=budget if budget is not None else config.seconds
        def persist_fast(policy: Policy, responses: list, audit: dict) -> None:
            elapsed=time.perf_counter()-started
            completed=int(elapsed//slice_seconds)
            if audit['status']!='searching':completed=max(1,int(np.ceil(elapsed/slice_seconds)))
            audit.update(completed_slices=completed,slice_seconds=slice_seconds,seconds=elapsed,
        ...
        if config.production_rescue:
            ...
            if checkpoint:save_snapshot(checkpoint/'latest.json',signature,accepted.policy,accepted.audit)
            return accepted
        if require:raise LimitedSolve(audit)
        return Solution(policy,responses,audit)
