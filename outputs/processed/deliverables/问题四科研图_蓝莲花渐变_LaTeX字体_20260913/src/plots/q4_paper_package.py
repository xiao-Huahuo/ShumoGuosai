...
from .q4_paper_data import ROOT,OUT,DATA

DOCS=ROOT/'docs/plots/q4_paper_final'
PACKAGE=ROOT/'outputs/processed/deliverables/问题四科研图_蓝莲花渐变_LaTeX字体_20260913'


def prepare():
    PACKAGE.mkdir(parents=True,exist_ok=True)
    for path in OUT.iterdir():
        if path.is_file() and path.suffix in ['.png','.svg','.json','.md','.html']:shutil.copy2(path,PACKAGE/path.name)
    shutil.copytree(DATA,PACKAGE/'data',dirs_exist_ok=True)
    for name in ['ThemeDisplayer.html','问题四科研图方案.md','图后分析与使用建议.md','acceptance.md']:
        shutil.copy2(DOCS/name,PACKAGE/name)
    folder=PACKAGE/'src/plots';folder.mkdir(parents=True,exist_ok=True)
    for name in ['q4_paper_data.py','q4_paper_final.py','q4_paper_verify.py','q4_paper_package.py']:
        shutil.copy2(ROOT/'src/plots'/name,folder/name)
    for p in [PACKAGE/'src/__init__.py',folder/'__init__.py']:p.write_text('',encoding='utf-8')
    (PACKAGE/'requirements.txt').write_text('numpy==2.3.5\npandas==2.2.3\nmatplotlib==3.10.8\nPillow==12.3.0\n',encoding='utf-8')
    readme='''# 问题四科研图：蓝莲花渐变与LaTeX字体
...
def archive():
    files=[p for p in sorted(PACKAGE.rglob('*')) if p.is_file() and p.name!='file_manifest.csv']
    with (PACKAGE/'file_manifest.csv').open('w',encoding='utf-8',newline='') as stream:
        w=csv.DictWriter(stream,fieldnames=['path','bytes','sha256']);w.writeheader()
        w.writerows({'path':str(p.relative_to(PACKAGE)),'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in files)
    ...
    result={'zip':str(target),'bytes':target.stat().st_size,'members':len(files),'PNG':6,'SVG':6,
            'sha256':hashlib.sha256(target.read_bytes()).hexdigest(),'CRC_and_member_SHA256':'passed'}
    (DOCS/'zip_acceptance.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8');print(result)
