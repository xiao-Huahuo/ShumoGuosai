...
NODES = (0, 6, 12, 18)
TOL = 1e-6
MODEL_VERSION = "q4_rescue_lp_v3"
THROUGHPUT_EPSILON = 1e-7
...
class Config:
    ...
    def __post_init__(self) -> None:
        ...
        if self.residual_window < 1:
            raise ValueError("残差窗口必须为正；正式运行预先固定56日")
        ...
        if self.hard_timeout <= 0 or self.seconds <= 0 or not 0 <= self.gap < 1 or self.solver_threads < 1:
    ...
    def signature(self) -> str:
        ...
        for source in production_sources():
            if not source.name.startswith("test_"):
                payload += str(source.relative_to(CODE_ROOT.parent)).encode("utf-8") + source.read_bytes()
        return hashlib.sha256(payload).hexdigest()


def production_sources() -> list[Path]:
    """覆盖实际Q4模块及只读输入解析的传递依赖；不依赖Q3求解器。"""
    return sorted(CODE_ROOT.glob("*.py")) + [
        CODE_ROOT.parent / relative for relative in
        ("q3/data.py", "q3/config.py", "q2/data.py", "q2/forecast.py", "q2/protocol.py", "q2/policy.py")
    ]
