...
## 问题四蓝莲花科研图（2026-09-13）

`q4_paper_data.py`只读正式4-2 v2、4-3 v3的共同334日及既有敏感性。`q4_paper_final.py`使用用户ThemeDisplayer的Blue Lotus/月白深蓝、TeX Fandol宋体/Latin Modern/Computer Modern输出六图：双机制DRO、24h电价时钟与密度、有符号费用桥与ECDF、守恒能量流与额度拆分、风险日四联、成本—紧急风险局部路径。`q4_paper_verify.py`检查682原件SHA、物理/结算/流带/配对/密度/元数据；`q4_paper_package.py`打包并校验ZIP。

重绘：`python -m src.plots.q4_paper_final --data-dir outputs/processed/figures/q4_paper_final_20260913/data --output-dir outputs/processed/figures/q4_paper_final_20260913`。字体来自本机TeX Live2026或解压目录assets/q4_fonts；现成SVG已转曲，不依赖字体。渐变嵌入栅格色带，轮廓和文字为矢量。
