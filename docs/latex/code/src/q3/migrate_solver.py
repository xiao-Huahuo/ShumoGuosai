...
from .fast_solver import build_matrix_v2_free, matrix_digest
...
def migrate(old: Path, new: Path, threads: int) -> dict:
    ...
    values=dict(original['config']);values['nodes']=tuple(values['nodes'])
    values.setdefault('solver_backend','highs');values.setdefault('formulation','legacy')
    before=Config(**values)
    after=replace(before,solver_threads=threads,solver_backend='highs',formulation='v2_free')
    ...
    for legacy_slice in (new/'main/nodes').glob('*/*/slice_*.json'):
        legacy_slice.unlink()
    ...
    for p in sorted((old/'main/nodes').glob('*/*/latest.json')):
        ...
        if audit.get('reliable'):
            migrated_audit={**audit,'migrated_from_request_signature':old_request,
                'certificate_origin_formulation':audit.get('formulation_version','legacy'),
                'migration_replay_objective_verified':True}
        else:
            matrix=build_matrix_v2_free(scenes,prices,initial,reference=reference,today=today,terminal=terminal)
            migrated_audit={'status':'migrated_policy_only_bound_reset','objective':cost,'lower_bound':None,
                'gap':None,'reliable':False,'completed_slices':0,'seconds':0.,'solver_threads':threads,
                'solver':'pending_HiGHS','solver_backend_requested':'highs','formulation_version':'v2_free',
                'matrix_digest':matrix_digest(*matrix[:4]),'bound_sources':[],
                'migrated_from_request_signature':old_request,'legacy_bound_discarded':audit.get('lower_bound'),
                'migration_replay_objective_verified':True}
        save_snapshot(new/p.relative_to(old),request,policy,migrated_audit)
    ...
    report.update(solver_backend='highs',solver_version='1.15.1',formulation_version='v2_free',
        legacy_search_slices_copied=False,unreliable_node_bounds_reset=True)
